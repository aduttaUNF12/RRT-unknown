...
global node_pos parent edge_wt cost_goal neighbours line_handles rhs r_radius o_nodes discovered_obstacles;
global goal_handles children  start_idx temp_edge_wt queue delta curr_node obstacle_cost;

startT = tic;% initialized time 
p_length = 0;  % Initialize path length
start = [1 1];       goal = [99 99];        epsilon = 1;         delta = 0;        ball_radius = (8)^2;
...
time_limit = 70;                                              line_handles = gobjects(samples,1);
goalAchieved=0;                     %tic;    
...
%   If the cost to goal has decreased,then rslot the goal line
...
%     o_nodes = getObstacleNodes(obstacles);
    ...
    p_length = p_length + pdist2(node_pos(prev_node), node_pos(curr_node), 'euclidean'); % get path length from one node to another
...
elapsedTime = toc(startT); % stop timing 
disp(['The function took ', num2str(elapsedTime), ' seconds to run']); % diaplsy total time in seconds
disp(['Total path length: ', num2str(p_length)]); % display the total path distance
