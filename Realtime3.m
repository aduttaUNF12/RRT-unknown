...
for run = 1:1
    ...
    EPS = 0.5;                % RRT* max connection distance
    numNodes = 100*10^3;      % RRT* iterations on global plan
    validationDist = 0.01;    % state validator resolution (meters)
    obstaclenum = 6;
    tic;

    %% ========= MAP =========
    simpleMap = zeros(1000, 1000, 'uint8');   % 0 = free, 1 = occupied
    map = occupancyMap(simpleMap, 10);
    Density = numNodes./(100*100);            % node density per m^2

    %% ========= START & GOAL =========
    start = [0.5 0.5 0];   % [x y theta] meters/radians (world frame, origin bottom-left)
    goal  = [99 99 0];
    ...
    %% ========= ADD ALL OBSTACLES =========
    % Each obstacle is [y, x, w, h] in CELLS (matrix rows, matrix cols, width cols, height rows)
switch obstaclenum
    case 6
    newObstacles = {
      [500 50 50 500];
    ...
    newPath = path;
    currentIndex = 1;
    res = 10;                          % cells per meter
    mapRows = size(simpleMap,1);       % 1000

    while currentIndex < size(newPath.States, 1)
        currentPos = newPath.States(currentIndex,:);
        nextPos    = newPath.States(currentIndex+1,:);  % [x y theta] (world meters)

        % ---- Collision check at NEXT waypoint
        if checkCollision(map, nextPos)
            fprintf('⚠️  Collision at step %d — local replanning...\n', currentIndex);
            ...
            % Convert nextPos (world meters) -> matrix indices (row, col)
            % row = mapRows - round(y*res); col = round(x*res)
            ...
            if ~matchFound
                % Fallback: nearest bbox center in matrix space
                minDist = inf;
                for k = 1:length(newObstacles)
                    obs = newObstacles{k};
                    oy = obs(1); ox = obs(2); ow = obs(3); oh = obs(4);
                    rcCenter = [oy + oh/2, ox + ow/2];   % [rowC, colC]
            ...
            % Obstacle center in MATRIX cell coordinates (rows down, cols right)
            rowC = oy + oh/2;
            colC = ox + ow/2;

            % Robust radius in CELLS: half diagonal + margin
            margin_cells = 20;                           % keep your margin
            ...
            [replannedSegment, ~, ~] = planPath(localMap, s, g, EPS, numNodesLocal, validationDist);
            ...
            x_center = (colC - 0.5) / res;                 % col -> X (m)
            y_center = (mapRows - (rowC - 0.5)) / res;     % row -> Y (m) single flip
        ...
        % advance along (possibly updated) path
...
function [path, solutionInfo, sv] = planPath(map, start, goal, EPS, numNodes, validationDist)
    ...
    planner = plannerRRTStar(ss, sv, ...
        ...
        GoalReachedFcn=@(~,s,g)(norm(s(1:2)-g(1:2)) < 0.01), ...
        GoalBias=0.1);
...
% Build a local map that is navigable ONLY inside a circle centered at rectCenterRC (matrix rc).
% - Outside the circle -> set to 1 (occupied)
% - Inside the circle   -> copy from simpleMap (keep true obstacles)
% Ensures s,g are inside the circle by expanding radius if needed.
